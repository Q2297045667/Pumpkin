extern "C" __global__ void interpolated_noise_sample_f64(
    const double* pos,
    const double* noise_perms_data, const double* lower_perms_data,
    const double* upper_perms_data,
    const double* noise_amps, const double* noise_lacs,
    const double* noise_orgs,
    const double* lower_amps, const double* lower_lacs,
    const double* lower_orgs,
    const double* upper_amps, const double* upper_lacs,
    const double* upper_orgs,
    ...
    double* res, int N, int noise_M, int lower_M, int upper_M
) {
    ...
    double n_sum = 0.0;
    const unsigned char* np = (const unsigned char*)noise_perms_data;
    ...
    if (q < 1.0) {
        const unsigned char* lp = (const unsigned char*)lower_perms_data;
    ...
    if (q > 0.0) {
        const unsigned char* up = (const unsigned char*)upper_perms_data;
    ...
    double clamped_q = fmin(fmax(q, 0.0), 1.0);
