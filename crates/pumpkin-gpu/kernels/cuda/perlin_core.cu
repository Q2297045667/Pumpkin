// CUDA-compatible Perlin core helpers
__constant__ double GRADIENTS[16][3] = {
...
__device__ double perlin_fade(double t) {
...
__device__ double maintain_precision(double value) {
    return value - floor(value / 33554432.0 + 0.5) * 33554432.0;
}

__device__ double sample_no_fade_core(
    const unsigned char* perm, double ox, double oy, double oz,
