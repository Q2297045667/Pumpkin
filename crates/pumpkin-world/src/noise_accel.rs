...
impl NoiseAccelerator {
    ...
    /// 批量填充 GPU 噪声缓存。
    ///
    /// 将采样器的配置和位置列表批量上传到 GPU，
    /// 计算所有噪声值并写入线程本地缓存。
    /// 后续 `OctavePerlinNoiseSampler::sample` 将直接从缓存读取。
    pub fn fill_noise_cache(
        &mut self,
        sampler_id: u64,
    ...
    /// 将批量采样结果写入线程本地缓存。
    fn insert_into_cache(sampler_id: u64, positions: &[f64], results: &[f64]) {
        let mut cache: std::collections::HashMap<(u64, i64, i64, i64), f64> =
            std::collections::HashMap::with_capacity(results.len());
        for i in 0..results.len() {
            let ix = (positions[i * 3] * 1_000_000.0) as i64;
            let iy = (positions[i * 3 + 1] * 1_000_000.0) as i64;
            let iz = (positions[i * 3 + 2] * 1_000_000.0) as i64;
            cache.insert((sampler_id, ix, iy, iz), results[i]);
        }
        pumpkin_util::noise::perlin::set_noise_cache(cache);
    }
}
