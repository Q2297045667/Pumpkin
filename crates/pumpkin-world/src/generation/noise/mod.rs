...
pub struct ChunkNoiseGenerator<'a> {
    ...
    /// GPU 批量加速器 —— 为 Cell Cache、Aquifer、Beardifier 提供批量采样。
    ...
    /// 布局：`[cache_index][cell_flat_index * hb² * vb + local_index]`
    #[cfg(feature = "gpu")]
...
impl<'a> ChunkNoiseGenerator<'a> {
    ...
    pub fn on_sampled_cell_corners(&mut self, cell_x: i32, cell_y: i32, cell_z: i32) {
        ...
        if let Some(ref chunk_cache) = self.gpu_cell_cache {
            ...
            let vc = self.vertical_cell_count;
    ...
    ///
    /// 收集当前区块所有 cell 的所有角点位置，一次性调用 GPU `batch_fill_cell_caches_vanilla`，
