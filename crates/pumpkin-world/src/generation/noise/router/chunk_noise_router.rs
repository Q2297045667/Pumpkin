...
impl<'a> ChunkNoiseRouter<'a> {
    ...
    pub fn fill_cell_caches(
        ...
        if let Some(accel) = crate::gpu::get_batch_accel()
            && let Some(specs) = self.build_cell_cache_fill_specs()
        {
    ...
    pub fn fill_interpolator_buffers(
        ...
        #[cfg(feature = "gpu")]
        if let Some(accel) = crate::gpu::get_batch_accel()
            && let Some(specs) = self.build_interpolator_fill_specs()
        {
            // 从第一个插值器获取缓冲区大小
            let total_positions = self
...
/// 返回组件的简洁类型标签（用于诊断日志）。
#[cfg(feature = "gpu")]
const fn component_tag(component: &ChunkNoiseFunctionComponent<'_>) -> &'static str {
    match component {
